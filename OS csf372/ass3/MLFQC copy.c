...
int scheduler_do(int time, scheduler *s1)
{
    ...
    int has_arrived = 0;
    if (s1->q1[0].front != -1 && s1->q1[0].arr[s1->q1[0].front].arr_time < time)
    {
        has_arrived = 1;
    }
    else if (s1->q1[0].front == -1)
    {
        has_arrived = 1;
    }
    //remove cputime from all procs
    if (s1->h1.n != 0)
    {
    ...
    for (int i = 2; i >= 1; i--)
    {
        if (s1->q1[i].front != -1)
        {
            if ((i == 0 && has_arrived == 1) || (i != 0))
            {
                s1->counter[i]++;
                s1->q1[i].arr[s1->q1[i].front].cpu_time--;
            }
        }
    }
    if (s1->q1[0].front != -1)
    {
        if (s1->q1[0].arr[s1->q1[0].front].cpu_time == 0)
        {
            process pre;
            pop(&s1->q1[0], &pre);
            final_times[pre.index] = time;
            s1->counter[0] = 0;
        }
        if (has_arrived == 1)
        {
            s1->counter[0]++;
            s1->q1[0].arr[s1->q1[0].front].cpu_time--;
        }
    }

    //check if ended
    if (s1->h1.n != 0 && s1->h1.arr[0].cpu_time == 0)
    {
        //printf("Process %d, has ended at time %d", s1->h1.arr[0].index, time);
        delete (&s1->h1);
    ...
    for (int i = 2; i >= 0; i--)
    {
        if (s1->q1[i].front != -1)
        {
            if (s1->q1[i].arr[s1->q1[i].front].cpu_time == 0)
            {
                if (((i == 0) && (has_arrived)) || (i != 0))
                {
                    //printf("Process %d, has ended at time %d", s1->h1.arr[0].index, time);
                    s1->counter[i] = 0;
                    process preempt;
    ...
    for (int i = 2; i >= 0; i--)
    {
        if ((s1->counter[i] == tq[i]) && s1->q1[i].front != -1)
        {
            pop(&s1->q1[i], &preempt);
            s1->counter[i] = 0;
            if (preempt.priority != -1 && preempt.cpu_time != 0)
            {
                if (i == 2)
